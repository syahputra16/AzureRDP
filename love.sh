...
image=$(cat win)
size=$(cat size)
rs=$(cat rs) && az vm create --resource-group $rs --name Windows-VM-PLUS --image $image --public-ip-sku Standard --size $size --location $location --admin-username rocky --admin-password RockyDares123 --nic-delete-option delete --os-disk-delete-option delete --out table
